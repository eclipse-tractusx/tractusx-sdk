...
This is a DID-based variant that uses do_get() instead of do_get_with_bpnl().
Use this when you already know the provider's DID and counter_party_address,
bypassing the need for BPNL-based discovery.

Flow:
1. Provider provisions data (Asset + Policies + Contract Definition)
2. Consumer calls do_get() with provider DID — SDK handles everything internally
...
_test_case_name = "tck_e2e_saturn_0-11-X_simple_did"
_now = datetime.now()
...
# ============================================================================
# PHASE 2 — Consumer: do_get does everything in one call (DID-based, no BPNL discovery)
# ============================================================================
...
def consume(
    ...
    """
    Phase 2: Consumer discovers and consumes data using do_get (DID-based).
    
    Args:
        consumer: Initialized consumer service
        asset_id: The asset ID to consume
        provider_config: Optional provider config (for DID, DSP URL). If None, uses PROVIDER_CONFIG.
    ...
    _sep("PHASE 2: Consumer — do_get (DID-based)")
    logger.info("Calling do_get() with provider DID — SDK handles catalog → negotiate → transfer → EDR → GET")
    ...
    logger.info("[DO_GET REQUEST]: counter_party_id=%s, asset_id=%s, path=/", provider_config["did"], asset_id)
...
def _print_summary(steps: list[dict], overall_result: str, total_elapsed: float,
    ...
    lines = [
        "",
        "╔" + "=" * (W - 2) + "╗",
        "║" + "  E2E TEST RUN SUMMARY  (simple DID-based)".center(W - 2) + "║",
        "╠" + "=" * (W - 2) + "╣",
...
def main(
    ...
    """
    Main execution flow: Provider provisions data, Consumer consumes it using DID.
    ...
    _sep("Saturn Connector Services — Simple E2E (DID-based)")
    ...
    try:
        ...
        response = _run_step("Phase 2 · do_get (catalog→negotiate→transfer→EDR→GET)",
        ...
        logger.exception("✗ Simple E2E (DID-based) FAILED: %s", e)
        all_phases = [
            "Phase 0 · Upload sample data to backend",
            "Phase 1 · Provider data provision",
            "Phase 2 · do_get (catalog→negotiate→transfer→EDR→GET)",
        ]
...
if __name__ == "__main__":
    _parser = argparse.ArgumentParser(
        description="Saturn Simple E2E example (DID-based variant)",
