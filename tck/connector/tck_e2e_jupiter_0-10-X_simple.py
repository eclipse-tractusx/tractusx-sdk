...
"""
Simple E2E Example: Jupiter Connector Services - Data Provision and Consumption
...
Jupiter uses the legacy DSP protocol version "dataspace-protocol-http" (EDC v0.8.x /
EDC v0.9.x era), together with the Tractus-X v1.0.0 and W3C ODRL JSON-LD contexts.
This is in contrast to Saturn, which uses "dataspace-protocol-http:2025-1" and the
Catena-X 2025-9 context URLs.
...
For the full step-by-step version see: jupiter_e2e_connector_service.py
For the Saturn (DSP 2025-1) equivalent see: saturn_simple_e2e.py
"""
...
_test_case_name = "tck_e2e_jupiter_0-10-X_simple"
_now = datetime.now()
...
#
# Jupiter catalog offers return policies with full odrl: prefixes and cx-policy: operand IDs.
# Example — restrict to the specific constraints provisioned above:
#   ACCEPTED_POLICIES = [{
#       "odrl:permission": {
...
#                   {"odrl:leftOperand": {"@id": "cx-policy:UsagePurpose"},       "odrl:operator": {"@id": "odrl:eq"}, "odrl:rightOperand": "cx.core.industrycore:1"},
#               ]
#           }
#       },
#       "odrl:prohibition": [],
...
def provision(
    ...
    ts = int(time.time())
    asset_id         = f"jupiter-e2e-asset-{ts}"
    access_policy_id = f"jupiter-e2e-access-{ts}"
    usage_policy_id  = f"jupiter-e2e-usage-{ts}"
    contract_def_id  = f"jupiter-e2e-contract-{ts}"
...
#            (Jupiter legacy DSP — "dataspace-protocol-http")
# ============================================================================
...
def consume(
    ...
    logger.info("Protocol: %s (Jupiter legacy DSP)", ConnectorConsumerService.DSP_0_8)
...
def _print_summary(steps: list[dict], overall_result: str, total_elapsed: float,
    ...
    lines = [
        "",
        "╔" + "=" * (W - 2) + "╗",
        "║" + "  E2E TEST RUN SUMMARY  (Jupiter · simple)".center(W - 2) + "║",
        "╠" + "=" * (W - 2) + "╣",
...
def main(
    ...
    _sep("Jupiter Connector Services — Simple E2E  (legacy DSP / dataspace-protocol-http)")
...
if __name__ == "__main__":
    _parser = argparse.ArgumentParser(
        description="Jupiter Simple E2E example (legacy DSP)",
