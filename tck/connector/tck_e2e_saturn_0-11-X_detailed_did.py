...
"""
End-to-End Example (DID-based): Saturn Connector Services - Data Provision and Consumption

This is a DID-based variant that uses the provider's DID directly, skipping BPNL discovery.
Use this when you already know the provider's DID and counter_party_address.
...
2. Consumer discovers available offers (Catalog Request using DID - no BPNL discovery)
3. Consumer negotiates contract
4. Consumer initiates transfer
5. Consumer accesses data using EDR

For the BPNL-based version see: tck_e2e_saturn_0-11-X_detailed.py
...
_test_case_name = "tck_e2e_saturn_0-11-X_detailed_did"
_now = datetime.now()
...
def initialize_provider_service(provider_config=None):
    """
    Initialize the Provider Connector Service following the Industry Core Hub pattern.
    
    Args:
        provider_config: Optional provider configuration dict. If None, uses PROVIDER_CONNECTOR_CONFIG.
...
def initialize_consumer_service(consumer_config=None, connection_manager=None):
    """
    Initialize the Consumer Connector Service following the Industry Core Hub pattern.
    
    Args:
        consumer_config: Optional consumer configuration dict. If None, uses CONSUMER_CONNECTOR_CONFIG.
...
def provision_data_on_provider(
    ...
    try:
        ...
        logger.info("[USAGE POLICY REQUEST]:\n%s", json.dumps(usage_policy_payload, indent=2))
...
def consume_data_as_consumer(consumer_service, asset_id, provider_config=None, consumer_config=None):
    """
    Phase 2: Consumer discovers and consumes data from Provider (DID-based, no BPNL discovery).
    
    Args:
        consumer_service: Initialized ConnectorConsumerService
        asset_id: The asset ID to consume
        provider_config: Optional provider config (for DID, DSP URL). If None, uses PROVIDER_CONNECTOR_CONFIG.
    ...
    if consumer_config is None:
        consumer_config = CONSUMER_CONNECTOR_CONFIG
    print_header("PHASE 2: Consumer Data Consumption (DID-based)")
    ...
    try:
        ...
        logger.info("[CATALOG REQUEST]:\n%s", json.dumps(catalog_request_params, indent=2))
        
        catalog_data = consumer_service.get_catalog_by_asset_id(
            counter_party_id=provider_config["did"],
            counter_party_address=provider_config["dsp_url"],
            asset_id=asset_id
        )
        ...
        # Extract participantId from catalog response (use DID as fallback)
        participant_id = catalog_data.get("participantId", catalog_data.get("participantId", provider_config["did"]))
        logger.info("  - Participant ID from catalog: %s", participant_id)

        # Extract DSP endpoint URL from catalog service
        dsp_endpoint = provider_config["dsp_url"]  # fallback
        ...
        logger.info("  - Provider DID: %s", provider_config['did'])
...
def _print_summary(steps: list[dict], overall_result: str, total_elapsed: float,
    ...
    lines = [
        "",
        "╔" + "=" * (W - 2) + "╗",
        "║" + "  E2E TEST RUN SUMMARY (DID-based)".center(W - 2) + "║",
        "╠" + "=" * (W - 2) + "╣",
...
if __name__ == "__main__":
    _parser = argparse.ArgumentParser(
        description="Saturn E2E connector service example (DID-based variant)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  # Run with default configuration from file
  python tck_e2e_saturn_0-11-X_detailed_did.py
  
  # Override provider DID and consumer URL
  python tck_e2e_saturn_0-11-X_detailed_did.py \\
    --provider-did did:web:provider.example.com \\
  ...
  python tck_e2e_saturn_0-11-X_detailed_did.py \\
    ...
    _parser.add_argument("--provider-did", help="Provider DID (e.g., did:web:provider.example.com)")
    ...
    if _args.provider_url:
        ...
        provider_config["management_url"] = _args.provider_url + "/management"
    ...
    if _args.provider_did:
        provider_config["did"] = _args.provider_did
    
    consumer_config = CONSUMER_CONNECTOR_CONFIG.copy()
    if _args.consumer_url:
        consumer_config["base_url"] = _args.consumer_url
        consumer_config["management_url"] = _args.consumer_url + "/management"
