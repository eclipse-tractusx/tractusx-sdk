...
class DspTools:
    ...
    def filter_assets_and_policies(catalog: dict, allowed_policies: list = []) -> list[tuple[str, dict]]:
        ...
        dataset: list | dict = catalog.get(DCATKeys.DATASET)
        ...
        if isinstance(dataset, dict):
            ...
            valid_assets.append((dataset.get(JSONLDKeys.AT_ID), policy))
            return valid_assets
        ...
        for item in dataset:
            ...
            if policy is not None:
                valid_assets.append((item.get(JSONLDKeys.AT_ID), policy))
    ...
    def is_catalog_empty(catalog: dict) -> bool:
        ...
        if isinstance(dataset, dict):
            return False if JSONLDKeys.AT_ID in dataset else True

        return len(dataset) == 0

    @staticmethod
    def get_dataset_policy(dataset: dict, allowed_policies: list = []) -> dict | None:
        policies: dict | list = dataset.get(ODRLKeys.POLICY, None)

        if isinstance(policies, dict):
            return policies if DspTools.is_policy_valid(policy=policies, allowed_policies=allowed_policies) else None
    ...
    def is_policy_valid(policy: dict, allowed_policies: list = []) -> bool:
        ...
        to_compare: dict = policy.copy()

        del to_compare[JSONLDKeys.AT_ID], to_compare[JSONLDKeys.AT_TYPE]
