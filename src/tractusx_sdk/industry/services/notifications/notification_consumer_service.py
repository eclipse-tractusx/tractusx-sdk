...
class NotificationConsumerService:
    ...
    def discover_notification_assets(
        ...
        try:
            ...
            # Extract datasets from catalog (supports both Saturn and Jupiter keys)
            datasets = _get_datasets(catalog)
            if datasets is None:
                datasets = []
    ...
    def discover_notification_assets_with_bpnl(
        self,
        bpnl: str,
        ...
        """
        Discover DigitalTwinEventAPI assets using BPNL-based connector discovery.
        
        Uses ``get_catalog_by_dct_type_with_bpnl`` which, in Saturn connectors,
        resolves the actual connector address and protocol via the BPNL first.
        
        Args:
            bpnl: Business Partner Number of the provider
            counter_party_address: DSP endpoint URL of the provider connector
            timeout: Request timeout in seconds (default: 60)
            dct_type: DCT type to filter by (default: DigitalTwinEventAPI)
        ...
        if self.verbose:
            self.logger.info(
                f"Discovering notification assets via BPNL {bpnl} "
                f"at {counter_party_address}"
            )
        
        try:
            catalog = connector.get_catalog_by_dct_type_with_bpnl(
                bpnl=bpnl,
    ...
    def negotiate_notification_access_with_bpnl(
        self,
        bpnl: str,
        counter_party_address: str,
        policies: Optional[List[Dict]] = None,
        ...
        """
        Negotiate notification access using BPNL-based connector discovery.
        
        Uses ``do_dsp_with_bpnl`` which, in Saturn connectors, resolves the
        actual connector address and protocol via the BPNL first.
        ...
        if self.verbose:
            self.logger.info(
                f"Negotiating notification access via BPNL {bpnl}"
            )
        ...
        try:
            ...
            endpoint, token = connector.do_dsp_with_bpnl(
                bpnl=bpnl,
                counter_party_address=counter_party_address,
                filter_expression=filter_expression,
                policies=policies,
            ...
            if self.verbose:
                self.logger.info(f"Obtained notification endpoint via BPNL: {endpoint}")
            
            return endpoint, token
            
        except Exception as e:
            self.logger.error(f"Failed to negotiate notification access via BPNL: {e}")
            raise NotificationError(f"Failed to negotiate notification access via BPNL: {e}")
    ...
    def get_notification_endpoint_with_bpnl(
        self,
        bpnl: str,
        counter_party_address: str,
        policies: Optional[List[Dict]] = None,
        ...
        """
        Get the notification endpoint using BPNL-based connector discovery.
        ...
        if self.verbose:
            self.logger.info(
                f"Getting notification endpoint via BPNL {bpnl}"
            )
        ...
        try:
            ...
            self.logger.error(f"Failed to get notification endpoint via BPNL: {e}")
            raise NotificationError(f"Failed to get notification endpoint via BPNL: {e}")
    ...
    def send_notification_with_bpnl(
        self,
        bpnl: str,
        counter_party_address: str,
        notification: Notification,
        ...
        """
        Send a notification using BPNL-based connector discovery.
        ...
        if self.verbose:
            self.logger.info(
                f"Sending notification {notification.header.message_id} "
                f"to BPNL {bpnl} via DSP"
            )
        
        try:
            # Get endpoint and token through DSP with BPNL discovery
            endpoint, token = self.get_notification_endpoint_with_bpnl(
                bpnl=bpnl,
                counter_party_address=counter_party_address,
                policies=policies,
            )
            ...
            self.logger.error(f"Failed to send notification via BPNL: {e}")
            raise NotificationError(f"Failed to send notification via BPNL: {e}")
